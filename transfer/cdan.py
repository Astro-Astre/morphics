...
class Transfer:
    ...
    def train(self, train_source_loader, train_target_loader, val_loader, domain_adv):
        ...
        for epoch in range(self.config.epochs):
            ...
            # eval_loss, eval_kl = self.eval(val_iter)
            self.scheduler.step(train_loss + train_kl)
            # print("Epoch: {}, eval loss: {:.4f}, eval kl: {:.4f}".format(epoch, eval_loss, eval_kl))
            self.save_checkpoint(epoch)
            self.early_stopping(train_loss + train_kl, self.model)
